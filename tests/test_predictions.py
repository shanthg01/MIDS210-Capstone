...
def test_requires_auth(client):
    assert client.get("/api/predictions?player_id=101&school_id=301").status_code == 401


def test_returns_200(client, H):
    assert client.get("/api/predictions?player_id=101&school_id=301", headers=H).status_code == 200
...
def test_schema(client, H):
    data = client.get("/api/predictions?player_id=101&school_id=301", headers=H).json()
    for field in (
...
def test_confidence_in_range(client, H):
    confidence = client.get("/api/predictions?player_id=101&school_id=301", headers=H).json()["confidence"]
    assert 0 <= confidence <= 1


def test_role_is_valid_enum(client, H):
    role = client.get("/api/predictions?player_id=101&school_id=301", headers=H).json()["predicted_role"]
    assert role in VALID_ROLES


def test_predicted_minutes_positive(client, H):
    assert client.get("/api/predictions?player_id=101&school_id=301", headers=H).json()["predicted_minutes"] > 0


def test_similar_transfers_nonempty(client, H):
    transfers = client.get("/api/predictions?player_id=101&school_id=301", headers=H).json()["similar_transfers"]
...
def test_shap_explanations_present(client, H):
    shap = client.get("/api/predictions?player_id=101&school_id=301", headers=H).json()["shap_explanations"]
