...
def test_response_schema(client, H):
    ...
    for field in ("player_id", "recommendations", "total", "generated_at", "model_version"):
...
def test_item_schema(client, H):
    ...
    for field in ("school_id", "school_name", "conference", "overall_fit", "components", "reasoning"):
...
def test_component_scores_in_range(client, H):
    ...
    for item in items:
        ...
        for key in ("gap_match", "scheme_fit", "opportunity", "personal_fit"):
