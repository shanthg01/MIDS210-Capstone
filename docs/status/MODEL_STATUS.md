...
**Last updated:** July 1, 2026 (Playing Time full all-school 2027 write complete; Destination Projection first real end-to-end run complete: 454,790 rows, CV resid_std=2.967, @champion; 247Sports matching overhauled to 87-91% match rate)
...
| M9 Destination-Adjusted Projection | Complete baseline — `player-destination-proj-v1` first real end-to-end run 2026-07-01. 454,790 rows for target_season=2027, 2,125 training rows, CV total_resid_std=2.967 (R²=0.025-0.071). MLflow @champion. Served by `GET /api/players/{id}/projection?school_id=X`. Known blocker before coach-facing box-score display: destination value deltas are usable for review, but displayed per-game box stats are under review after the Jalik Dunkley sanity check surfaced a rate-basis mismatch (`*_per_40` neutral box fields scaled like per-100 possession rates) and likely over-aggressive single-player usage compression. Known gap: `estimate_usage_value_coef` returns fallback 1.5 every run (zero-overlap bug). | Fix per-game box-score translation basis; inspect Playing Time usage-budget compression for one-player counterfactual projections; fit style/skill delta empirically (biggest R² gain); fix usage-coef zero-overlap; position-specific competition tier effects; barttorvik as secondary RAPM label. Full roadmap in `../models/destination_projection_plan.md` §19. | [`../models/destination_projection_plan.md`](../models/destination_projection_plan.md); [`../../scripts/run_destination_projection.py`](../../scripts/run_destination_projection.py); [`../../src/portalpoint/modeling/destination_projection.py`](../../src/portalpoint/modeling/destination_projection.py) |
...
✅ Destination-Adjusted Proj     (player-destination-proj-v1; 454,790 rows, target=2027; CV resid_std=2.967; @champion)
→  Program Fit
...
`role_fit` is implemented in branch but not yet fully written for all 2027 pairs. `program_fit`
is still stubbed at 50. `scheme_fit` and `gap_match` are real and serve as 2026 fit-context inputs
for the 2027 Playing Time write.
...
|---|---|---|---|---|
| 4 | Playing Time / Rotation -> Role Fit | Draft implementation in branch; full 2027 all-school write pending | shared roster baseline, 2027 neutral player projection, 2026 fit/roster context | `playing_time_projections`, `player_team_fit_scores.role_fit` |
...
| 4b | Playing Time / Rotation -> Role Fit | Draft implementation in branch; full 2027 all-school write pending | shared roster baseline, neutral player projection, M1/M3 helpful | `playing_time_projections`, `player_team_fit_scores.role_fit` |
...
> **Updated:** `fit_scores.py` uses season-aware lookup and serves real `scheme_fit` + `gap_match`; `role_fit` is implemented in branch via `playing_time_projections` but still needs the full 2027 production write before the app should present it as real. `program_fit` remains a deterministic 50.0 placeholder until Program Fit exists.
