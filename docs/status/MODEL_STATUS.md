...
**Last updated:** June 25, 2026 (Player Projection production-integration decision made — API stays on Phase 0, Phase 2a tracked in MLflow as a real but sub-threshold candidate — see Player Projection section below and `../models/player_projection_state_space_plan.md` §22 for the full record)
...
| Player Projection (Model #8) | ✅ Phase 0 (`player-projection-shrinkage-v1`) in production since 2026-06-23, serves `GET /api/players/{id}/projection`. **Phase 2a implemented + real-data validated (2026-06-25):** beats Phase 0 on held-out offense every rolling-origin fold, ties on defense. Writes real rows under a separate `model_version="player-projection-phase2a-v1"` (Gap F) — never collides with Phase 0's rows. 11th skill (`foul_discipline`), offense/defense feature-set split, Gap A (shared-prior blending) and Gap E (archetype metadata) all landed and validated against real data. Gap B (observation-layer context adjustment) regressed accuracy on real data — flagged TBD, not enabled. **MLflow tracking added for Phase 2a 2026-06-25 — registered under the same `"player-projection"` model name as Phase 0, gated by the same `maybe_promote` 5% threshold: `Δ=+1.0%` (in-sample), does not clear the bar. Production-integration decision made: API's default stays on Phase 0** — the gate's the same one every other model in this codebase is held to, and this result doesn't clear it. | None — decision made. Revisit if Gap B's resolution or anything else pushes Phase 2a's real numbers past the 5% bar. | None blocking — all open items are roadmap, not MVP blockers. | [`../models/player_projection_state_space_plan.md`](../models/player_projection_state_space_plan.md) §22; [`../../scripts/run_player_projection.py`](../../scripts/run_player_projection.py) (`--phase {0,2a,both}`, both phases); [`../../notebooks/models/player_projection_state_space.ipynb`](../../notebooks/models/player_projection_state_space.ipynb) (Phase 0/1/2a, interactive/diagnostic) |
...
✅ Neutral Player Projection     (Phase 0 in production 2026-06-23; Phase 2a real-data validated 2026-06-25, beats Phase 0 on offense)
...
| Item | Current state |
|---|---|
| Phase 0 | ✅ Production since 2026-06-23. `player-projection-shrinkage-v1`. Empirical-Bayes shrinkage + Ridge value model vs. Hoop Explorer RAPM. 27,047+ neutral rows in `player_projections`. Served by `GET /api/players/{id}/projection`. |
...
| Phase 2a | ✅ Implemented, reconciled against [Issue #37](https://github.com/shanthg01/MIDS210-Capstone/issues/37), real-data validated 2026-06-25. Two-level Kalman (intra-season + cross-season persistence/drift). **Beats Phase 0 on held-out offense every fold; ties on defense.** Writes real rows under a separate `model_version="player-projection-phase2a-v1"` via Gap F — can never collide with Phase 0's rows (different partial-unique-index key). |
| Gap B (context adjustment) | ⚠️ Coded, real-data tested, **regresses accuracy** (worse than even Phase 0 on offense). Flagged TBD pending root-cause work — not enabled, not rejected outright. |
...
| Production integration | ✅ **Decided 2026-06-25.** API's hardcoded `model_version` default stays on Phase 0 — Phase 2a's real improvement (Δ=+1.0% in-sample, ~2.9% held-out) doesn't clear the same `maybe_promote` 5% gate every other model here is held to. Phase 2a now has real MLflow tracking (registered, Staging) for whenever that changes. |
| Test coverage | 194 tests (`test_player_projection*.py`, 4 files) — all passing. |
...
| 4a | Neutral Player Projection | ✅ Phase 0 in production; Phase 2a real-data validated (beats Phase 0 on offense), production-integration decision made (stays on Phase 0 — Phase 2a's real gain doesn't clear the standard 5% MLflow promotion gate) | player game logs or season-level fallback, HE impact labels | `player_projections` (both `player-projection-shrinkage-v1` and `player-projection-phase2a-v1` rows present) |
...
✅ Neutral Player Projection (Phase 0 production; Phase 2a real-data validated)
  -> Role Fit / Playing Time
  -> Destination-Adjusted Player Projection
  -> Program Fit
  -> fit_scores.py full scoring
