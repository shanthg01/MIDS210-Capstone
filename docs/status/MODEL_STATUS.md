...
**Last updated:** July 21, 2026 (M5 Transfer Success — first real run against shared RDS, 456,615 rows, 3 real bugs found/fixed getting there; see M5 row below). Previously: July 15, 2026 (`/api/recommendations` wired to the real `rec-v1.2` engine; Scheme Fit mislabel/fake-metric fixes + `he_scheme_fit` surfaced in the API/UI — see `../models/recommendation_engine_plan.md` and M3 section below)
...
| M5 Transfer Success | ✅ **First real run against shared RDS, 2026-07-21.** `transfer-success-eb-v1` — empirical-Bayes shrinkage over `(player_cluster, team_offense_cluster_id, team_defense_cluster_id)` cells, time-decay weighted (`DECAY_LAMBDA=0.9`), success labeled against destination-adjusted projection baseline (`value_per_100`, PER-improvement fallback). 456,615 rows written to `transfer_success_scores` (1,251 unique portal candidates × D1 schools, target_season=2027). Backtest: 2,734 completed transfers, 2,143 labeled, out-of-sample Brier=0.2492, overall success rate 53.5%. MLflow `transfer-success` v1 `@champion` (first production run, no prior baseline). `predictions.py`/`comparison.py` query real rows via `prediction_service.py`; `ComparePage.tsx` already rendered `success_probability`/`success_tier`/`explanation` and needed no changes. **Three real bugs found getting this run to actually work, all fixed:** (1) `score_active_candidates` concatenated an already-scored historical frame with the active frame, colliding on same-named columns inside `compute_success_probability`'s merge (`KeyError: 'cluster_success_rate'`); (2) `upsert_transfer_success_scores` used plain psycopg2 `executemany` for 456K rows — one round-trip per row over the SSM tunnel, ran 90+ minutes with no end in sight before being killed; replaced with `execute_values` (batched), matching `destination_projection.py`'s own pattern, final run completed in ~6-22 min; (3) `score_active_candidates` never called `attach_similar_transfers`/`build_explanation` on the forward-scored rows at all, so every real row would have written NaN into `explanation` (a required, non-Optional `PredictionResponse` field) — fixed, then a 4th layer surfaced once comps populated: NaN inside comp dicts (`minutes_drift`/`usage_drift`) broke `json.dumps` against Postgres's stricter JSON parser, fixed with a sanitization pass. 16 unit tests (up from 14). | None for MVP — done. | [`../../scripts/run_transfer_success.py`](../../scripts/run_transfer_success.py); [`../../src/portalpoint/modeling/transfer_success.py`](../../src/portalpoint/modeling/transfer_success.py); [`../../notebooks/models/transfer_success.ipynb`](../../notebooks/models/transfer_success.ipynb) |
