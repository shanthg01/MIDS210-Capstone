...
| M5 Transfer Success | **v2 inference + API (2026-07-21).** `transfer-success-eb-v2` — 3-level EB hierarchy, offense-pair fallback, projection covariate offset, calibration artifacts, chunked inference with season-level projection z-scores. Backtest Brier evaluated vs MLflow `@champion` v1 (0.2492); API serves `SERVING_MODEL_VERSION` (v2 only if backtest beats champion, else v1). **456,615 rows** for season=2027. `/api/predictions` wired via `prediction_service.py` (real/stub split). | Beat v1 Brier on same eval set; low-tier calibration; wire M5 into recommendation engine; Fit Score UI. | [`../../src/portalpoint/modeling/transfer_success.py`](../../src/portalpoint/modeling/transfer_success.py); [`../../scripts/run_transfer_success.py`](../../scripts/run_transfer_success.py); [`../../tests/test_transfer_success.py`](../../tests/test_transfer_success.py) |
