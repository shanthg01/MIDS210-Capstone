...
**Last updated:** July 21, 2026 (M5 Transfer Success v2 — Blocks 2–6 shipped; inference + API wiring complete)
...
| M5 Transfer Success | **v2 inference + API live (2026-07-21).** `transfer-success-eb-v2` — 3-level EB hierarchy, offense-pair fallback, projection covariate offset, calibration artifacts. Backtest Brier **0.2521** (n_eval=1755, `player-destination-proj-v1`); MLflow `@champion` remains v1 (0.2492) — champion is backtest bookkeeping only; inference writes and the API serve v2 rows from `transfer_success_scores`. **456,615 rows** for season=2027 with non-null `explanation` and `similar_transfers`. `/api/predictions` wired via `transfer_success_service.py`. | None for MVP. | Per-level K tuning; beat v1 Brier on same eval set; low-tier calibration; wire M5 into recommendation engine; Fit Score UI. | [`../../src/portalpoint/modeling/transfer_success.py`](../../src/portalpoint/modeling/transfer_success.py); [`../../scripts/run_transfer_success.py`](../../scripts/run_transfer_success.py); [`../../tests/test_transfer_success.py`](../../tests/test_transfer_success.py) |
...
✅ M5 Transfer Success v2        (3-level EB + covariate offset; 456k inference rows; `/api/predictions` wired 2026-07-21)
