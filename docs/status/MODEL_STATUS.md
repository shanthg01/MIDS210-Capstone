...
| M5 Transfer Success | **v2 inference + API gate (2026-07-23).** `transfer-success-eb-v2` — 3-level EB hierarchy, offense-pair fallback, projection covariate offset, calibration artifacts, chunked inference with season-level projection z-scores (chunk-boundary bug fixed 2026-07-23). Backtest Brier **0.2521** (n_eval=1755); MLflow `@champion` v1 **0.2492** — API serves `SERVING_MODEL_VERSION`=`transfer-success-eb-v1` until v2 beats champion. v2 inference rows refreshed after z-score fix. `/api/predictions` wired via `prediction_service.py` (real/stub split). | Beat v1 Brier on same eval set; low-tier calibration; wire M5 into recommendation engine; Fit Score UI. | [`../../src/portalpoint/modeling/transfer_success.py`](../../src/portalpoint/modeling/transfer_success.py); [`../../scripts/run_transfer_success.py`](../../scripts/run_transfer_success.py); [`../../tests/test_transfer_success.py`](../../tests/test_transfer_success.py) |
