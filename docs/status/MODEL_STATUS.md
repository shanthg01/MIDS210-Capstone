...
**Last updated:** June 27, 2026 (Playing Time / Role Fit implementation drafted and season contract corrected for live 2026 portal-cycle scoring: 2026 completed stats/roster context -> 2027 projected playing-time rows; Player Projection production semantics remain Phase 2a next-season forecast)
...
| M4 Role Fit / Playing Time | Draft implementation in branch: `playing-time-rotation-v2` trains supervised minutes-share/usage models, writes `playing_time_projections`, derives usage role/displacement/uncertainty, and upserts target-season `player_team_fit_scores.role_fit` from all-pairs fit-context rows. Live season contract is explicit: `target_season=2027` uses 2026 completed source stats, 2026 roster snapshots/baseline, 2026 gap/scheme/program fit context, and 2027 neutral player projections. Duke-only dry run scored 4,548 candidates; focused tests pass. Full all-school 2027 production write still pending. | Run full all-school 2027 write and inspect score/coverage distributions before presenting Role Fit as production; keep all candidate-school rows written, with recommendation/API filtering handled separately. | Add scenario controls for minutes/usage/displaced players; improve high-minute/high-usage tail recall and interval calibration; add richer team-style interactions. | [`../models/playing_time_rotation_model_plan.md`](../models/playing_time_rotation_model_plan.md); [`../../scripts/run_playing_time.py`](../../scripts/run_playing_time.py); [`../../src/portalpoint/modeling/playing_time.py`](../../src/portalpoint/modeling/playing_time.py) |
