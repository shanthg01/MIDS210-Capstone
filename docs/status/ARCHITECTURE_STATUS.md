...
**Gap Matching is now `gap-cos-v3`, all-pairs (2026-06-22):** scores every eligible player×school×season pair (9,731,957 rows), not just whatever `scheme-cos-v3` Scheme Fit pre-seeded. Both `scripts/run_gap_matching.py` and `notebooks/models/gap_matching.ipynb` filter out players who transferred out of a school (per `transfers`, current season only) before computing that school's gap vectors — `gap_matching.filter_departed()`, notebook Cell 1b. Scoped to *portal* departures specifically (`transfers.from_school_id`/`to_school_id`) — not graduations or draft entries, which `roster_state_features.open_minutes_by_position` (broader: anyone not returning, any reason) covers separately but isn't wired into Gap Matching. Both re-executed end to end and produce identical numbers — verified, not just claimed. `player_team_fit_scores.is_portal_candidate` (migration `c7f1a9d3e652`) flags rows whose player is currently in the transfer portal that season, kept in sync by `portalpoint.modeling.availability` — see `MODEL_STATUS.md` and CLAUDE.md Process Improvement TODO #5 for full detail, including two bugs (role_fit/program_fit placeholder mismatch, an O(table) preload that didn't scale) found and fixed while rebuilding at this scale.
...
13. run_gap_matching.py                            # gap-cos-v3 — all-pairs, departure-aware via transfers, current season only
```
