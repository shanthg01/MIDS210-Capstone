...
| Cache | Docker Redis on port `6379`, local dev only — **confirmed broken in production 2026-07-20** (`REDIS_URL` never added to `task-def.json`). Two different failure modes found on real code, not the same bug: (1) `fit_score_service`'s cache-aside layer fails **open** to raw DB queries — real incident, fixed at the query layer instead (new index), Redis itself intentionally left unfixed. (2) `api/routers/agent.py`'s news-monitoring run-tracking (lock + status, `POST`/`GET /api/agent/news-monitoring/*`) has **no fail-open path** — those Redis calls now fail **closed** (503, PR #65 review) rather than crashing with a raw 500, but the endpoints still don't work without Redis. **Open decision, deferred on purpose:** either make `agent.py` fail-open too (skip the concurrency lock, tolerate unknown run-status — real work still completes via Postgres either way) or stand up real ElastiCache. Revisit together, not piecemeal — same underlying gap. | Defer — not needed for POC-scale traffic once the index exists (item 1); item 2 blocks the news-monitoring *API* specifically until one of the two options above is picked |
