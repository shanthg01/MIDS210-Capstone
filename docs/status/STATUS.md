...
Start with `MODEL_STATUS.md` for the current critical path. M1 (re-trained, tuned group-weighted), M3, and Gap Matching are complete; `fit_scores.py` serves real `scheme_fit` + `gap_match` from `player_team_fit_scores` with dynamic current-season resolution. M2 has been rewritten to the same tuned group-weighted architecture (targets the old C7 degenerate-cluster issue) but **needs a re-run** before its labels are trustworthy. Role Fit (M4) is the next critical-path model after that.
