...
**Last updated:** July 21, 2026 (`predictions.py`'s Transfer Success model run for real against shared RDS for the first time — see `MODEL_STATUS.md`'s M5 row). Previously: July 16, 2026 (news-monitoring agent wired for manual live runs — CLI, RDS migrations, DB write pipeline, integration tests)
...
| `predictions.py` | Real, live | Transfer Success model (`transfer-success-eb-v1`) queries `transfer_success_scores` via `api/services/prediction_service.py` (real/stub split, mirrors `fit_score_service.py`); stub fallback only for player/school pairs with no scored row. `schemas/prediction.py` reshaped to the model's real empirical-Bayes output (`success_probability`/`success_tier`/`explanation`/`similar_transfers`) — no SHAP fields, this isn't a tree model. **First real run against shared RDS 2026-07-21** — 456,615 rows now live (was empty/stale before this date); see `MODEL_STATUS.md`'s M5 row for the 3 real bugs found getting that run to actually populate real `explanation`/`similar_transfers` values. |
| `projections.py` | Real | Team Rating Projection (M6, PR #49, merged to `main` 2026-07-11) — `/team-rating/top` (ranked roster-impact list) and per-pair fetch both query `team_rating_projections` directly. Confirmed real by direct read 2026-07-19 — this doc previously said "Stub on `main`," which was stale even before that merge landed. |
| `comparison.py` | Real (prediction piece fixed 2026-07-19) | Player/fit-score halves were already real DB (`_player_info`, `fit_score_service`); the `prediction` field used a hardcoded `_stub_prediction()` until 2026-07-19, now wired to the same `prediction_service.get_prediction()` as `predictions.py`. |
