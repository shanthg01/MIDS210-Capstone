...
| `predictions.py` | Real, live | Wired via `prediction_service.py` — reads non-expired `transfer_success_scores` rows for `SERVING_MODEL_VERSION` (v2 after backtest gate; falls back to v1 if v2 does not beat `@champion`). Real/stub split mirrors `fit_score_service.py`. |
