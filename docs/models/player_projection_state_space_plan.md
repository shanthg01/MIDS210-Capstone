...
### Real production bug found and fixed (2026-07-15): `build_cross_season_records` silently dropped ~99% of real box scores, including live production

Surfaced while running the Destination Projection historical backtest (`docs/models/destination_projection_backtest_plan.md`) — the backtest's headline finding was a uniform +8.5 PPG "underprojection" across every position/archetype/tier, too consistent to be model inaccuracy. Traced to the source:

**Root cause:** `player_id` is a 63-bit BigInteger (`hash(barttorvik_id)`). `build_cross_season_records()` built `rates_lookup` (Gap C's per-player rate dict, keyed by `(player_id, season)`) via `for _, rr in projected_rates_df.iterrows(): rates_lookup[(int(rr["player_id"]), int(rr["season"]))] = ...`. `DataFrame.iterrows()` upcasts an entire row to one dtype — `projected_rates_df` is purely numeric (`player_id`/`season` plus only `rate_*` float columns, no string column), so every row upcasts to `float64`, and any `player_id >= 2^53` (~9.0×10^15) loses precision. Since `player_id` values are ~uniformly distributed over 63 bits, only ~1/1024 (~0.1%) survive the round-trip exactly. Confirmed live at 3 scales: an isolated single-row test, a 300-player proxy run, and a 3,000-player/8,265-row proxy run — real box-score `nonempty` rate was **1.40%**, matching the real DB's ~0.3-0.5% across every season checked (2021-2027, including live production).

**Why the written `player_id` itself stayed correct (a real, if accidental, mitigation) while the box score was still empty:** `projected_df` (the *other* frame `build_cross_season_records` iterates, supplying the record's own `player_id`/`season`/skill fields) includes a `position` column — a string. That one string column forces the *whole row* to upcast to `object` dtype instead of `float64`, which preserves the original Python int exactly. So every written `player_projections` row got the correct `player_id` — only the `rates_lookup` *key* (built from the purely-numeric `projected_rates_df`) was corrupted, causing the lookup to miss and `projected_box_score` to fall back to `{}` for ~99% of rows. Same root defect class as the `roster_state_features.py` player_id-corruption bug found earlier the same session, via a different, previously-unknown mechanism (`iterrows()`'s automatic row-dtype promotion, not `pd.DataFrame()`'s column-dtype inference).

**Fix:** `build_cross_season_records` now extracts `player_id`/`season` from each frame's typed columns via `.to_numpy(dtype=np.int64)` before the loop (for `projected_rates_df`, `archetypes_df`, and `projected_df` itself — the last one wasn't actually broken given the accidental `position`-column protection, but was fixed too rather than leaving correctness dependent on an incidental column happening to be present). Verified on the real 3,000-player/8,265-row proxy population using the actual (fixed) function: **nonempty box-score rate went from 1.40% to 100.00%**, with real, sensible per-40 values (e.g. `pts_per_40: 12.41-14.93`, `reb_per_40: ~5.1-5.4`). 2 new regression tests (`test_build_cross_season_records_preserves_bigint_player_id_through_rates_lookup`, `test_build_cross_season_records_multiple_bigint_players_no_cross_contamination`), 67 tests green in `test_player_projection.py`.

**Scope/impact:** this affects every `player-proj-phase2a-fcast-v1` row ever written — every historical season *and* the live 2027 production target. `value_per_100` (the ranking metric) is unaffected; only the displayed per-game box-score fields were empty. **Not yet done:** the real production re-run (`scripts/run_player_projection.py --phase cross-season`, a genuine multi-hour job) needed to backfill correct box scores into the live table — this fix only corrects the code going forward, not the ~33,540 already-written rows.

**Related, not fixed here:** `build_neutral_records` (Shrinkage Baseline, `player-projection-shrinkage-v1`) and the `archetype_lookup` construction both use the same `int(r["player_id"])`-from-`iterrows()` pattern. `build_neutral_records`'s `df` likely also carries a `position`/string column (same accidental protection), and Shrinkage Baseline has been extensively spot-checked as correct in production — left as-is rather than changing working, validated code without direct evidence of a live problem there. Worth a follow-up check, not urgent.
